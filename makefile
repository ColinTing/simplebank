...
migrateup1:
	migrate -path db/migration -database "postgres://root:112233@localhost:5432/simple_bank?sslmode=disable" -verbose up 1

migratedown:
	migrate -path db/migration -database "postgres://root:112233@localhost:5432/simple_bank?sslmode=disable" -verbose down

migratedown1:
	migrate -path db/migration -database "postgres://root:112233@localhost:5432/simple_bank?sslmode=disable" -verbose down 1
...
.PHONY: postgres createdb dropdb migrateup migrateup1 migratedown migratedown1 sqlc test cleantest server mock 